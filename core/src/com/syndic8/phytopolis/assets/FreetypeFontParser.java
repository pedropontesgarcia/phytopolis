        ...
        if (root.isString()) {
            ...
            if (params.fontFileName.toLowerCase().endsWith(".ttf")) {
                uid = uid.substring(0, uid.length() - 4) + ":" +
                        params.fontParameters.size + ".ttf";
            }
            ...
            params.fontParameters.hinting = ParserUtils.parseHinting(root.get(
                    "hinting"), FreeTypeFontGenerator.Hinting.AutoMedium);
            params.fontParameters.color = ParserUtils.parseColor(root.get(
                    "color"), Color.WHITE);
            params.fontParameters.gamma = root.getFloat("gamma", 1.8f);
            params.fontParameters.renderCount = root.getInt("renderCount", 2);
            params.fontParameters.borderColor = ParserUtils.parseColor(root.get(
                    "borderColor"), Color.BLACK);
            params.fontParameters.borderStraight = root.getBoolean(
                    "borderStraight",
                    false);
            params.fontParameters.borderGamma = root.getFloat("borderGamma",
                                                              1.8f);
            params.fontParameters.shadowOffsetX = root.getInt("shadowOffsetX",
                                                              0);
            params.fontParameters.shadowOffsetY = root.getInt("shadowOffsetY",
                                                              0);
            params.fontParameters.shadowColor = ParserUtils.parseColor(root.get(
            ...
            params.fontParameters.characters = root.getString("characters",
                                                              FreeTypeFontGenerator.DEFAULT_CHARS);
            params.fontParameters.kerning = root.getBoolean("kerning", true);
            params.fontParameters.flip = root.getBoolean("flip", false);
            params.fontParameters.genMipMaps = root.getBoolean("mipmaps",
                                                               false);
            params.fontParameters.minFilter = ParserUtils.parseFilter(root.get(
                    "minFilter"), Texture.TextureFilter.Linear);
            params.fontParameters.magFilter = ParserUtils.parseFilter(root.get(
