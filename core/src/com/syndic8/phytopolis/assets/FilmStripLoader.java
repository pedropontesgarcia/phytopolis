...
public class FilmStripLoader
        extends AsynchronousAssetLoader<FilmStrip, FilmStripLoader.FilmStripParameters> {
    ...
    public void loadAsync(AssetManager manager,
                          String fileName,
                          FileHandle file,
                          FilmStripParameters params) {
        Texture texture = manager.get(manager.getDependencies(fileName)
                                              .first());
     ...
     */
    @Override
    public FilmStrip loadSync(AssetManager manager,
                              String fileName,
                              FileHandle file,
                              FilmStripParameters params) {
    ...
    public FilmStrip load(Texture texture, FilmStripParameters params) {
        ...
        int w = params.width == -1 ?
                texture.getWidth() - params.x :
                params.width;
        int h = params.height == -1 ?
                texture.getHeight() - params.y :
                params.height;
        FilmStrip result = new FilmStrip(texture,
                                         params.rows,
                                         params.cols,
                                         params.size,
                                         params.x,
    ...
    public FileHandle resolve(String fileName) {
        ...
        if (suffix == -1) {
            throw new GdxRuntimeException(
                    "Texture region file name must end in " +
                            FilmStripParser.ALIAS_SEP + "name'.");
        }
