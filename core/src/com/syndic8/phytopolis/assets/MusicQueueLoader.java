...
public class MusicQueueLoader
        extends AsynchronousAssetLoader<MusicQueue, MusicQueueLoader.MusicQueueParameters> {
    ...
    public void loadAsync(AssetManager manager,
                          String fileName,
                          FileHandle file,
                          MusicQueueParameters params) {
        if (params == null) {
            params = new MusicQueueParameters();
        }

        if (params.sources.size == 0) {
            cachedBuffer = ((AudioEngine) Gdx.audio).newMusicBuffer(params.isMono,
                                                                    params.sampleRate);
        } else {
            AudioSource first = manager.get(manager.getDependencies(fileName)
                                                    .first(),
                                            AudioSource.class);
            cachedBuffer = ((AudioEngine) Gdx.audio).newMusicBuffer(
                    first.getChannels() == 1, first.getSampleRate());
        }
     ...
     */
    @Override
    public MusicQueue loadSync(AssetManager manager,
                               String fileName,
                               FileHandle file,
                               MusicQueueParameters params) {
